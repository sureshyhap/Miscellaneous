...
template <typename T>
void final_sort(std::vector<T>& lyst, std::vector<T*> indirect);

template <typename T>
inline ptrdiff_t index_from_pointer(const std::vector<T>& data, const std::vector<T*>& pointers, int index_pointers);

int main(int argc, char* argv[]) {
  std::vector<int> v = {60, 20, 70, 30, 10, 80, 40, 90, 50};
  std::vector<int> v1 = {4, 1, 3, 6, 78, 3, 5, 2, 6, 3, 23, 1};
  int size = v.size();
...
void final_sort(std::vector<T>& lyst, std::vector<T*> indirect) {
  ...
  for (int i = 0; i < size; ++i) {
    indices[i] = index_from_pointer(lyst, indirect, i);
  }
  for (int i = 0; i < size; ++i) {
    if (indices[i] == i) {
      continue;
...
template <typename T>
inline ptrdiff_t index_from_pointer(const std::vector<T>& data, const std::vector<T*>& pointers, int index_pointers) {
  return pointers[index_pointers] - &data[0];
}
